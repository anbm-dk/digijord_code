...
rsaga.geoprocessor('grid_calculus', module = 6,
                   env = work_env,
                   param = list(TARGET_OUT_GRID = 'DEM_random.sgrd',
                                ITERATIONS = 1000))

rsaga.geoprocessor('io_gdal', module = 2,
                   env = work_env,
                   param = list(GRIDS = list('DEM_random.sgrd'),
                                FILE = 'DEM_random.tif'))
...
letstry <- sample_kmeans(input = dem
                         , use_xy = TRUE
                         , clusters = n_clusters
                         , scale = TRUE
                         , weights = dist
...
cl_colors <- expand.grid(r = seq(0, 1, 0.05)
                         , g = seq(0, 1, 0.05)
                         , b = seq(0, 1, 0.05)) %>% 
...
sortednames <- c('bluespot', 'MRVBF', 'SAGAWI', 'DEM', 'valleydepth', 'ECa'
...
letstry2 <- sample_kmeans(input = vin_sel
                          , use_xy = TRUE
                          , pca = TRUE
                          , n_pcs = 5
                          , clusters = 100
