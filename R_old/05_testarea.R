...
extr_basic <- read.table(file = 'dsc_top_extr_basic.csv'
                         , sep = ';'
                         , header = TRUE
)
...
extr_veg <- read.table(file = 'dsc_top_extr_veg.csv'
                       , sep = ';'
                       , header = TRUE
)
...
extr_radar <- read.table(file = 'dsc_top_extr_radar.csv'
                         , sep = ';'
                         , header = TRUE
)
...
cov_grid <- expand_grid(basic = 0:1
                        , bare = 0:1
                        , series = 0:1
                        , radar = 0:1
) %>%
...
# for(i in 1:nrow(cov_grid))  # i is the row in the covariate grid
{
  ...
  for(k in 1:ncol(pts_top_tex))  # k is is texture class in the vector
  {
    ...
    all_RMSEs %>%
      write.table(file = rmse_file
                  , sep = ';'
                  , row.names = FALSE
      )
    
    all_R2s %>%
      write.table(file = r2_file
                  , sep = ';'
                  , row.names = FALSE
      )
  ...
  outname <- paste0('bigtest', testn, '_imps_'
                    , formatC(i
                              , width = cov_grid %>%
                                nrow %>%
                                nchar
...
for(i in 1:ncol(all_R2s))
{
  cov_tib_i <- cov_tib %>%
    ...
    arrange(R2) %>%
    mutate(cov_collapsed = sapply(Covariates
                                  , function(x) paste0(sort(x)
                                                       , collapse = "-"))) %>%
    mutate(cov_collapsed = fct_reorder(cov_collapsed, R2, .desc = TRUE))
  
  tiff(filename = paste0(plots, 'R2_', i, '_', colnames(all_R2s)[i], '.tif')
       , units = 'cm'
       , width = 16*1.5
       , height = 10*1.5
       , res = 600
...
for(i in 1:ncol(all_RMSEs))
{
  ...
  tiff(filename = paste0(plots, 'RMSE_', i, '_', colnames(all_RMSEs)[i], '.tif')
       , units = 'cm'
       , width = 16*1.5
       , height = 10*1.5
       , res = 600
...
all_R2s %>% apply(2, function(x) max(x, na.rm = TRUE)) %>% plot
all_RMSEs %>% apply(2, function(x) min(x, na.rm = TRUE)) %>% plot
...
pdf(file = 'bigtest', testn, '_importance.pdf'
    , paper = 'a4'
    , width = 0,
    height = 0)
