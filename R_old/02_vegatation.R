...
weeks_df <- data.frame(year = year(all_extrs_dates)
...
nas_week %>% lapply(sum) %>% unlist %>% plot
...
for(i in 1:length(all_dates))
{
  ...
  if(length(l) > 0)
  {
    ...
    extr_i %>% write.table(file = outname
                           , sep = ';'
                           , row.names = FALSE)
  }
}
...
maxcors <- l_cors %>% lapply(max) %>% unlist
mincors <- l_cors %>% lapply(min) %>% unlist
...
cors_all <- l_cors %>%
  ...
  lapply(function(x) add_column(x
                                , texclass = texclas)) %>%
  lapply(function(x) pivot_longer(x
                                  , cols = !texclass
                                  , names_to = 'date_band')) %>%
  bind_rows() %>%
...
tiff(filename = 'correlation_bands.tiff'
     , units = 'cm'
     , width = 16*1.5
     , height = 10*1.5
     , res = 600
...
r2s <- models_veg %>% lapply(get_r2) %>% unlist
...
RMSEs <- models_veg %>% lapply(get_RMSE) %>% unlist
...
imps %>%
  ...
  pivot_wider(values_from = mean
              , names_from = date) %>%
  write.table(file = 'imps_veg_date.csv'
              , sep = ';')
...
imps %>%
  ...
  pivot_wider(values_from = max
              , names_from = date) %>%
  write.table(file = 'imps_veg_date_max.csv'
              , sep = ';')
...
imps %>%
  ...
  pivot_wider(values_from = mean
              , names_from = date) %>%
  write.table(file = 'imps_veg_date_band.csv'
              , sep = ';')

imps_wide <- imps %>% pivot_wider(values_from = Overall
                                  , names_from = tclass)

imps_wide %>% write.table(file = 'imps_veg_all.csv'
                          , sep = ';'
                          , row.names = FALSE)
